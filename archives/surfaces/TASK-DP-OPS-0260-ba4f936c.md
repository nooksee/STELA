...
previous: archives/surfaces/TASK-DP-OPS-0259-4c74c05f.md
---
...
### DP-OPS-0260: Add synthetic regression coverage for dp lint delete-load-order seams
...
Opening OPEN Artifact: storage/handoff/OPEN-main-ba4f936c.txt
Opening OPEN Trace ID: stela-20260410T014929Z-703c0edc
...
Not applicable. This packet adds synthetic regression coverage and directly coupled spec text; it does not change live dp lint enforcement semantics.
...
Objective:
Add the first dedicated explicit-path regression harness for `tools/lint/dp.sh`, focused on the delete/load-order contradiction seam that currently exists only inside the internal `bash tools/lint/dp.sh --test` path.

In scope:
- `tools/test/dp.sh`: add a canonical PASS fixture and a deterministic FAIL fixture for a `3.4.3 DELETE` path that still appears in `3.2.2 DP-scoped load order`.
- `docs/ops/specs/tools/lint/dp.md`: update regression-coverage language so the external harness is named truthfully.
- directly coupled allowlist maintenance only if verification proves it is required.

Out of scope:
- `tools/lint/dp.sh` semantic changes unless verification proves a minimal fixtureability shim is strictly necessary.
- broader DP lint failure classes beyond the first delete/load-order seam.
- certify replay, audit traversal, or non-DP testing expansion.
...
- Do not reopen RESULTS lint, OPEN authority, or closeout sidecar law in this packet.
...
### 3.4.1 State
- `main` is at `ba4f936c`.
- `tools/test/results.sh` now covers the strict explicit-path RESULTS receipt and narrative-hygiene seams that recently failed in live packets.
- `tools/lint/dp.sh` already enforces delete/load-order contradictions and already covers that case in `bash tools/lint/dp.sh --test`.
- There is still no dedicated repo-native `tools/test/dp.sh` harness that exercises that seam through explicit-path lint invocation.

### 3.4.2 Request
- Add a dedicated `tools/test/dp.sh` entrypoint for explicit-path DP lint regression coverage.
- First slice only:
  - canonical PASS fixture for a clean DP
  - deterministic FAIL fixture for a `DELETE` path that still appears in `3.2.2 DP-scoped load order`
- Update the dp lint spec so the external harness is documented alongside the internal `--test` fixtures.
- Leave `tools/lint/dp.sh`, `ops/src/surfaces/dp.md.tpl`, `tools/lint/task.sh`, and `ops/bin/certify` untouched unless verification proves a minimal helper shim is strictly necessary.

### 3.4.3 Changelog
- ADD tools/test/dp.sh
- UPDATE docs/ops/specs/tools/lint/dp.md
- UPDATE storage/dp/active/allowlist.txt

### 3.4.4 Patch / Diff
1. Add `tools/test/dp.sh` with one canonical PASS fixture and one deterministic delete/load-order contradiction FAIL fixture.
2. Keep the harness direct: generate synthetic DP fixtures and call `bash tools/lint/dp.sh <fixture>` explicitly.
3. Update `docs/ops/specs/tools/lint/dp.md` so the regression-coverage section names the dedicated external harness truthfully.
4. Add `tools/test/dp.sh` to `storage/dp/active/allowlist.txt`.
5. Prove that `tools/lint/dp.sh`, `ops/src/surfaces/dp.md.tpl`, `tools/lint/task.sh`, and `ops/bin/certify` stayed untouched.
...
- rg -n 'DELETE|DP-scoped load order|tools/test/dp.sh|explicit-path|canonical PASS fixture|deterministic FAIL fixture' tools/test/dp.sh tools/lint/dp.sh docs/ops/specs/tools/lint/dp.md
