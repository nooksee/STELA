...
previous: archives/surfaces/TASK-DP-OPS-0245-1a55577b.md
---
...
### DP-OPS-0246: Thin non-planning stance verifier duplication without changing rendered output
...
Tooling-applicable. This packet changes the active non-planning stance verifier in `tools/lint/style.sh` and the coupled stance/style specs. The required discipline is to preserve the runtime stance owners (`ops/src/stances/draft.md.tpl`, `ops/src/stances/audit.md.tpl`, `ops/src/stances/addenda.md.tpl`, plus `ops/src/shared/stances.json`) while thinning only verifier and mirror duplication around them.
...
Objective:
- Continue stance normalization beyond `0245` by extending the runtime-owner / verifier / mirror cleanup to the non-planning stance family.
- Thin duplicate non-planning stance checks in `tools/lint/style.sh` and align the coupled draft, audit, addenda, and style specs without changing rendered stance output.
- Keep the packet conservative: verifier/spec thinning only, no runtime stance-template edits.

In scope:
- `tools/lint/style.sh`
  - Thin `check_draft_mode_contract()` and `check_addenda_mode_contract()` from sentence-by-sentence freeze into smaller invariant-family checks where grouped checks preserve the same contract.
  - Normalize the remaining audit/addenda style-side checks so the verifier protects the required non-planning contract anchors without pretending to co-own every sentence.
- `docs/ops/specs/tools/lint/style.md`
  - Mirror the slimmer non-planning stance enforcement model exactly.
- `docs/ops/specs/stances/draft.md`
  - State the owner split plainly for the draft stance.
- `docs/ops/specs/stances/audit.md`
  - State the owner split plainly for the audit stance.
- `docs/ops/specs/stances/addenda.md`
  - State the owner split plainly for the addenda stance.
- `storage/dp/active/allowlist.txt`
...
- Preserve rendered draft, audit, and addenda stance output in this packet; do not modify `ops/src/stances/draft.md.tpl`, `ops/src/stances/audit.md.tpl`, `ops/src/stances/addenda.md.tpl`, or `ops/src/shared/stances.json`.
- Reduce verifier/spec duplication first; do not rewrite runtime-owned stance text in this slice.
- Keep non-planning behavior stable: draft remains single-fence DP output, audit remains single-fence verdict output, and addenda remains single-fence addendum output with required headings and decision coherence.
- No scope expansion into planning or response-envelope behavior unless directly required to keep touched specs or lint truthful.
...
### 3.4.1 State
- `0245` established the runtime-owner / verifier / mirror pattern for planning without touching `ops/src/stances/planning.md.tpl` or `ops/src/shared/stances.json`.
- The remaining active stance weight is still on the non-planning style side: `tools/lint/style.sh` continues to freeze draft, audit, and addenda contracts as line-by-line ownership rather than grouped invariant families.
- The coupled draft, audit, addenda, and style specs do not yet state the owner split explicitly for those non-planning stances.

### 3.4.2 Request
- Keep this packet conservative and non-behavioral.
- Treat the non-planning stance templates and shared stance include as runtime owners and leave them untouched in this slice.
- Refactor the non-planning checks in `tools/lint/style.sh` so they validate bounded invariant families instead of sentence-by-sentence prose duplication wherever that can be done without relaxing the actual contract.
- Update the coupled stance/style specs so they describe the runtime-owner / verifier / mirror split truthfully for draft, audit, and addenda.
- If any proposed change would require touching `ops/src/stances/draft.md.tpl`, `ops/src/stances/audit.md.tpl`, `ops/src/stances/addenda.md.tpl`, or `ops/src/shared/stances.json`, STOP and leave it for a later packet.
...
### 3.4.4 Patch / Diff
1. Inspect the live non-planning owner chain: `ops/src/stances/draft.md.tpl`, `ops/src/stances/audit.md.tpl`, `ops/src/stances/addenda.md.tpl`, `ops/src/shared/stances.json`, `tools/lint/style.sh`, and the coupled stance/lint specs.
2. Rewrite the non-planning style checks so they guard the draft, audit, and addenda contracts by smaller invariant families instead of duplicating every exact line where a grouped check is sufficient.
3. Keep the existing non-planning invariants intact: draft single-fence DP output, audit single-fence verdict output, addenda required headings and decision coherence, and shared non-audit include coverage.
4. Update the draft, audit, addenda, and style specs to describe the runtime-owner / verifier / mirror split explicitly and remove stale sentence-level freeze expectations.
5. Update the allowlist for touched files, run `./ops/bin/llms`, and accept only the direct generated fallout if it appears.
...
- rg -n "runtime owner|verifier|mirror|owner split" docs/ops/specs/stances/draft.md docs/ops/specs/stances/audit.md docs/ops/specs/stances/addenda.md docs/ops/specs/tools/lint/style.md
