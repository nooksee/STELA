...
Certify opens the scaffold in `$EDITOR` (falling back to `vi`) and requires the worker to replace the scaffold text with actual execution narrative. After the editor exits, certify validates:
...
The validated narrative is passed as the `CONTRACTOR_NARRATIVE` slot to both template render invocations. The narrative appears in RESULTS between `## Git State Impact` and `## Mandatory Closing Block`.
