...
The binary initializes terminal color support, enforces repo-root execution, and accepts zero or one argument. With no argument it prints the command index and the complete topic-to-verdict cockpit quick start. In `specs` mode it recursively lists spec markdown files under `docs/ops/specs` grouped by top-level category with stable sort order. In `doctrine` mode it extracts Filing Doctrine, Axioms, Canon Surfaces, and read order lines from `PoT.md`. In `curriculum` mode it prints the runtime-neutral cockpit, role-boundary, and closeout command sequences. In search mode it runs fixed-string search across specs first, then scans `docs` excluding specs, and prints results in that order.
