...
`ops/bin/meta` exits on non-root execution, missing project argument, invalid project slug, extra arguments, missing project directory, missing or mismatched front-door policy keys, or any non-zero exit from `ops/bin/bundle`. The binary does not emit partial success output when bundle generation fails.
