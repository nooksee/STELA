...
The binary parses target selection (`sop`, `pow`, `both`, or `storage`), phase mode (`report` or `apply`), optional dry-run and scrub flags, then loads `ops/lib/manifests/PRUNE.md` before any candidate processing. Policy load is fail-closed: missing required keys or malformed values stop execution.
