...
`ops/bin/prune` exits on invalid target values, missing guard prerequisites, dirty tracked RESULTS and CLOSING artifacts, missing or untracked pointer targets in PoW prune candidates, and ledger prune failures. Untracked runtime handoff files do not satisfy evidence continuity by themselves and are ignored by `results_guard`. The threshold value is fixed in script state and the command does not infer missing pointer classes or repair malformed ledger entries.
