...
## Closeout Refresh vs Audit Dump
Two dump use cases are distinct and must not be conflated:
- **Operator session refresh:** `./ops/bin/open --out=auto` is the primary closeout refresh step. It re-establishes the session anchor for the next operator session. A standalone `ops/bin/dump --scope=core` in DP §3.5 closeout is not required and is not a universal obligation. The `dp.md.tpl` default closeout route does not include it.
- **Audit dump:** Generated by `./ops/bin/bundle --profile=audit --out=auto`. Bundle owns audit dump generation; the audit dump is a profile-scoped artifact under `storage/dumps/` with an artifact-aligned naming stem. Operators must use `bundle --profile=audit` for audit submission, not a standalone `ops/bin/dump` call.
