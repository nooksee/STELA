...
The binary parses mode and template type arguments, supports both `template <type>` and `template render <type>` call forms, emits tool telemetry leaf boundaries, and dispatches target groups. It routes `agent`, `task`, and `skill` requests to `ops/bin/factory` with the original argument vector, and routes `dp`, `results`, `task-surface`, `spec`, `addendum`, `closing`, and stance render requests to `ops/bin/manifest` with the same argument vector. Compatibility stance aliases are retained here as well, including `stance-analyst` -> `stance-planning`, so legacy render callers continue to resolve through the shim. Unknown or missing types terminate execution with explicit errors.
