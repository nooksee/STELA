...
`ops/bin/bundle` produces delivery packets for planning, draft, audit, project, and addenda profiles. It is a runtime-neutral transport contract, not a free-form export path: web attachment and exact repo-native file access expose the same generated artifacts, while profile routing, explicit disposable inputs, dump selection, and emitted artifact identity remain deterministic.

## Shipping Spine Position
Bundle carries three artifact transitions in the shipping spine:
- **Planning input:** `--profile=planning` delivers the context package that transforms `TOPIC.md` into `PLAN.md`.
- **Integrator draft input:** `--profile=draft` delivers the plan and authoring scaffold to the Integrator, who performs the Architect stance and produces the active DP draft at `storage/dp/intake/DP.md`.
- **Independent audit delivery:** `--profile=audit` packages certify-generated RESULTS and CLOSING for a review context independent of Worker execution. This is the canonical audit intake mechanism and is **separate** from operator session refresh (`ops/bin/open`, `ops/bin/dump` for CDD). Do not conflate bundle audit with standalone `ops/bin/dump --scope=core` closeout steps.
...
The planning, draft, and audit packages carry their complete launch instructions. The validated DP and its authorized dispatch artifacts do the same for Worker execution. Free-form Operator prompt text required to start any main-lane role run: `0`. A bounded planning clarification after launch remains valid. Runtime host identity does not change the role, evidence, or output contracts.
...
- `storage/handoff/PLAN.md`: planning output surface, latest-wins; the planning stance writes this file after each run.
...
- `storage/dp/intake/DP.md`: latest-wins active DP draft surface; printed in bundle `[REQUEST]` as `dp_draft_path`. The Integrator performs the Architect stance and emits a fenced worker-ready DP body saved here by the Operator or a repo-native Integrator for dispatch under fresh packet-scoped Worker authority.
...
Audit review is context-independent from Worker execution and returns the contracted `PASS` or `FAIL` report. Reusing a model or provider is permitted; reusing Worker conversation or unrecorded execution memory as evidence is not.
