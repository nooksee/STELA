...
`ops/bin/draft` exits on dirty trees, missing required arguments, malformed packet IDs, missing required slot content, missing slots file, template hash mismatch, malformed TASK Section 3 structure, or inability to locate the active DP heading for replacement. Addendum mode adds hard-stop exits for missing `--base-dp` when `--addendum` is present, missing `--addendum` when `--base-dp` is present, invalid addendum letter values, invalid base packet IDs, and missing base packet intake artifacts in both intake and processed storage. It does not continue after partial render or partial TASK rewrite conditions.
