...
> **Retired by DP-OPS-0102 (2026-02-22).** The deprecated-filename check was absorbed into ops/bin/llms. Bundle staleness is now prevented structurally via `.github/hooks/pre-commit`. This spec is retained as a historical record.
...
4. Create temporary output directory, run generator with `--out-dir`, and print generator output. The generator canonicalizes repo-root `--out-dir` variants before success-path rendering, so repository-root output paths in that success line remain repo-relative even when the provided root path carries trailing slashes.
