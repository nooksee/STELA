...
11. Parse `## Skill Bindings` and enforce explicit labels (``required_skills``, ``optional_skills``). Each list contains deterministic Skill paths under `opt/_factory/skills/` or the explicit literal `(none)`, never both.
