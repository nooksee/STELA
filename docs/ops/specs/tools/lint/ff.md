...
Phase 1 scoring is heuristic and proxy-based rather than entropy-derived, so false positives remain possible, especially on short files with unusual sentence distributions. Fenced-code exclusion for ASL and AAD is line-oriented and may miss nested or non-standard fence patterns. Phase 1 is the production enforcement model until a later governance decision supersedes it. Generated-file exemption relies on matching marker comments in the first five lines, so generated files without those markers may be scored.
