...
## Template Extraction Contract
- `ops/bin/prune --reset-task` extracts template content between these exact delimiter lines:
  - `<!-- TASK-TEMPLATE-BEGIN -->`
  - `<!-- TASK-TEMPLATE-END -->`
- Content between delimiters must be valid `TASK.md` and must pass `bash tools/lint/task.sh`.
- Delimiter lines are metadata and are not included in extracted output.

## Embedded Canonical TASK Template
Do not edit delimiter lines. Update template content only through governed TASK surface changes.
