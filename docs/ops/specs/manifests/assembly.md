...
5. On ATS success, bundle runtime emits a deterministic runtime pointer artifact under `storage/handoff/` and records pointer metadata at `assembly.pointer`.
6. On ATS failure, runtime exits non-zero before artifact emission.
...
- Runtime pointer emission when ATS is not applied.
...
Runtime pointer emission policy is controlled by `runtime_pointer_emit_mode`; default is `emit_when_applied` and pointer format is policy-defined (`runtime_pointer_format`, currently `json`).
