...
- Core planning contract:
  - Machine-ingest planning mode requires bundle-provided `storage/handoff/TOPIC.md` and has no inline-query fallback.
  - Planning uses delivered bundle evidence first.
  ...
  - The immediate packet is explicit only if the topic directly names the first packet or first work family, the delivered bundle evidence directly requires a first packet ordering, or the user explicitly prioritizes one work family.
  ...
  - Machine-ingest planning mode must not add repository-operating details, workflow examples, command families, or GitHub action lists unless they are directly visible in delivered bundle artifacts.
  - Machine-ingest planning mode keeps repo-specific claims generic when evidence is thin and makes concrete repo-specific claims when the relevant surfaces are delivered and directly visible.
- Portable clarification transport:
  ...
  - Planning clarification mode marks at most one substantive option per question `(Recommended)` and only when directly visible bundle evidence actually justifies it; the redirect option is never marked `(Recommended)`.
