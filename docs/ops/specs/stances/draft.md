...
- Input surface: `storage/handoff/PLAN.md` is the latest-wins plan input written by the planning stance (`Analyst`).
- Runtime transport is host-neutral: a web UI may receive the generated draft artifacts as attachments, while a repo-native Integrator may read the same exact files at their emitted paths.
- Empty user text is valid when all required draft artifacts are available; the generated package carries the instruction.
- `Architect` is the Integrator draft stance, not an additional staffing role or relay stage.
...
- Active DP draft surface: `storage/dp/intake/DP.md` is the latest-wins worker-ready DP surface saved directly from the Integrator Architect-stance output, then validated with `tools/lint/dp.sh` before dispatch.
...
The Integrator performs the Architect stance at the third artifact transition in the main shipping spine: `TOPIC.md` -> `PLAN.md` -> Integrator draft bundle -> embedded DP authoring scaffold -> `storage/dp/intake/DP.md` (active DP draft) -> packet-authorized Worker execution.
Integrator output is a worker-ready full DP body completed from the embedded authoring scaffold; the Operator or repo-native Integrator saves it to `storage/dp/intake/DP.md`, validates with `bash tools/lint/dp.sh storage/dp/intake/DP.md`, and dispatches the passing packet.
