...
The helper assumes caller context provides `REPO_ROOT` and `die` from `ops/lib/scripts/common.sh`. Editor command execution uses shell invocation and must be treated as trusted operator input. Validation rejects unchanged scaffold lines by exact match, so future scaffold-text edits must remain synchronized between scaffold writers and validators.
