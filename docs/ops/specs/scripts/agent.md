...
- Agent files in `docs/library/agents/`.
...
## Outputs
- `harvest`: writes redacted draft under `storage/archives/agents/` and appends candidate log in `docs/library/AGENTS.md`.
- `harvest-check`: prints Pattern Density report from recent SoP entries.
- `promote`: writes canon agent file in `docs/library/agents/`, inserts registry row in `docs/ops/registry/AGENTS.md`, and appends promotion log in `docs/library/AGENTS.md`.
