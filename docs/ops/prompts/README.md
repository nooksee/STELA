...
* **PLAN structure:** `ops/src/stances/plan.md.tpl`
...
* **Bundle stance templates:** `ops/src/stances/*.md.tpl`
...
* **Stance authority:** Bundle contract bodies are rendered from `ops/src/stances/*.md.tpl` via manifest stance keys.
...
* **For addendum authorization intake (Integrator/Foreman):** Use `./ops/bin/bundle --profile=foreman --intent="ADDENDUM REQUIRED: <DECISION_ID> - <ONE-LINE BLOCKER>" --out=auto` only.
* **For addendum artifact generation (Contractor):** Follow `docs/ops/prompts/e-prompt-06.md` decision-first flow (decision leaf, OPEN intent, core dump, foreman bundle).
* **For project-scoped context capture:** Use `./ops/bin/bundle --profile=project --project=<name> --out=auto`.
* **For conform normalization intake:** Use `./ops/bin/bundle --profile=conform --out=auto`.
...
| `foreman` | `./ops/bin/bundle --profile=foreman --intent="ADDENDUM REQUIRED: <DECISION_ID> - <ONE-LINE BLOCKER>" --out=auto` | `BUNDLE-*.txt`, `BUNDLE-*.manifest.json` | Addendum authorization only. |
...
| `conform` | `./ops/bin/bundle --profile=conform --out=auto` | `BUNDLE-*.txt`, `BUNDLE-*.manifest.json`, draft DP input | Structure-conformance normalization flow. |
...
**Stance Index:**
* **E-PROMPT-01:** Auditor (audit before merge)
* **E-PROMPT-02:** Conformist (bundle-first DP conformance)
* **E-PROMPT-03:** Architect (generate DP from plan handoff)
* **E-PROMPT-04:** Analyst (read-only analysis + architect-ready PLAN mode)
* **E-PROMPT-05:** Foreman (authorize addendum from foreman bundle)
...
* `foreman` profile is for addendum authorization only.
* Legacy `auditor` profile is a compatibility alias that resolves to `foreman`.
* Legacy `hygiene` profile is a compatibility alias that resolves to `conform`.
...
* PLAN Template: `ops/src/stances/plan.md.tpl`
