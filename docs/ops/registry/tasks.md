<!-- CCD: ff_target="operator-technical" ff_band="25-35" -->
# Task Registry

Authoritative registry for canon tasks.

| ID | Name | File Path | Notes |
| --- | --- | --- | --- |
| B-TASK-08 | Task: Bundle Orchestration | opt/_factory/tasks/b-task-08.md | |
| B-TASK-07 | Task: Factory Chain Validation | opt/_factory/tasks/b-task-07.md | |
| B-TASK-01 | Task: Code Review | opt/_factory/tasks/b-task-01.md | |
| B-TASK-02 | Task: Verification Command | opt/_factory/tasks/b-task-02.md | |
| B-TASK-03 | Task: Integrate Command | opt/_factory/tasks/b-task-03.md | |
| B-TASK-04 | Task: Plan Command | opt/_factory/tasks/b-task-04.md | |
| B-TASK-05 | Task: Refactor Clean | opt/_factory/tasks/b-task-05.md | |
| B-TASK-06 | Task: Update Documentation | opt/_factory/tasks/b-task-06.md | |
