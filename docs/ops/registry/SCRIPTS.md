...
| SCRIPT-07 | Retroactive Surface Leaf Migration | ops/lib/scripts/retroleaf.sh | One-time DP-OPS-0072 migration tool. Supports `--dry-run`, `--apply`, and `--verify` to split Phase 2 PoW/SoP dump leaves into per-entry chains, preserve dump backups, repair TASK previous-cycle drift, and emit tracked telemetry. |
