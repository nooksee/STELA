...
| SCRIPT-11 | Bundle Routing Library | ops/lib/scripts/bundle.sh | Spec: `docs/ops/specs/scripts/bundle.md`. Shared deterministic routing, architect slice validation, request metadata emission (`request.slice_id`, `request.slice_validated`, `request.plan_source`), embedded OPEN metadata generation, manifest v2 emission, and bundle package (`.tar`) composition logic used by `ops/bin/bundle`. |
