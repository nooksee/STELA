...
| SCRIPT-11 | Bundle Routing Library | ops/lib/scripts/bundle.sh | Spec: `docs/ops/specs/scripts/bundle.md`. Shared deterministic routing, direct architect plan intake, request metadata emission (`request.plan_source`, `request.packet_id`, `request.dp_draft_path`, `request.closing_sidecar`), embedded OPEN metadata generation, manifest v2 emission, and bundle package (`.tar`) composition logic used by `ops/bin/bundle`. |
| SCRIPT-12 | Editor Capture Library | ops/lib/scripts/editor.sh | Spec: `docs/ops/specs/scripts/editor.md`. Shared editor-resolution, scaffold orchestration, and deterministic validation helpers used by `ops/bin/certify` and `ops/bin/draft` scaffold assist modes. |
