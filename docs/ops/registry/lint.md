...
| LINT-17 | Response Lint | tools/lint/response.sh | Spec: `docs/ops/specs/tools/lint/response.md`. Enforces strict fenced-envelope intake for machine markdown ingress in `--mode=dp|audit`, applies marker and contamination-token rejection, and delegates structural DP validation in `dp` mode. Cross-stance output-contract rollout is tracked under Plan Track B (`B1`-`B4`). |
