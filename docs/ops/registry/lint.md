...
| LINT-17 | Response Lint | tools/lint/response.sh | important | Spec: `docs/ops/specs/tools/lint/response.md`. Enforces strict fenced-envelope intake for machine markdown ingress in `--mode=dp|audit|draft|planning|addenda|conformist`, aligns cross-stance freeze keys in `ops/src/shared/stances.json`, applies marker and contamination-token rejection, rejects draft/planning/addenda/conformist role-drift markers, enforces addenda addendum-structure checks plus decision-field coherence when present, and delegates structural DP validation in `dp`, `draft`, and `conformist` modes with deterministic draft delegate-on fixtures. |
