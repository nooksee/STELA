...
| LINT-16 | PLAN Lint | tools/lint/plan.sh | supporting | Spec: `docs/ops/specs/tools/lint/plan.md`. Deterministic safety-floor check for `storage/handoff/PLAN.md` used by bundle route gating; enforces the canonical final-plan heading set only. |
| LINT-17 | Response Lint | tools/lint/response.sh | important | Spec: `docs/ops/specs/tools/lint/response.md`. Enforces strict fenced-envelope intake for machine markdown ingress in `--mode=dp|audit|architect|planning|foreman|conformist`, aligns cross-stance freeze keys in `ops/src/shared/stances.json`, applies marker and contamination-token rejection, rejects architect/planning/foreman/conformist role-drift markers, enforces foreman addendum-structure checks plus decision-field coherence when present, and delegates structural DP validation in `dp`, `architect`, and `conformist` modes with deterministic architect delegate-on fixtures. |
