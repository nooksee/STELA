...
| LINT-05 | LLMS Lint | tools/lint/llms.sh | supporting | Retired by DP-OPS-0102. Deprecated-filename check absorbed into ops/bin/llms. Staleness protection moved into `.github/hooks/pre-commit`. |
...
| LINT-11 | RESULTS Lint | tools/lint/results.sh | critical | Spec: `docs/ops/specs/tools/lint/results.md`. Verifies certification RESULTS schema, template hash parity, Worker Execution Narrative requirements, and Decision Required/Decision Leaf coherence. Hash parity and decision coherence are strict for explicit or inferred-active targets; `--all` also scans durable archived RESULTS receipts and keeps historical compatibility findings non-blocking where specified. |
| LINT-12 | Schema Lint | tools/lint/schema.sh | supporting | Spec: `docs/ops/specs/tools/lint/schema.md`. Validates unified schema front-matter keys for `archives/definitions` leaves and Phase 2 surface leaves, enforces present-state SoP and TASK contracts, and requires current PoW to resolve durable RESULTS and packet proof or declare the exact legacy gap. |
