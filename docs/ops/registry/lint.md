...
| LINT-07 | Style Lint | tools/lint/style.sh | supporting | Spec: `docs/ops/specs/tools/lint/style.md`. Rejects markdown contractions across tracked documentation surfaces and enforces audit-versus-foreman mode split guard lines, audit output marker/no-citations lines, live compatibility alias deprecation key lines in the bundle policy manifest, and canonical OPEN marker contract lines in `ops/bin/open` (with legacy marker regression blocked). |
