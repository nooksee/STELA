...
| LINT-17 | Response Lint | tools/lint/response.sh | Spec: `docs/ops/specs/tools/lint/response.md`. Enforces raw-model response intake for both DP and audit (`--mode=dp|audit`): strict fenced-envelope + no-outside-text in both modes, marker validation (`### DP-` or `**AUDIT -`), and drift-token rejection; delegates DP body validation to `tools/lint/dp.sh` in `dp` mode. |
