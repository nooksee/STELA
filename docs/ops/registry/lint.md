...
| LINT-17 | Response Lint | tools/lint/response.sh | Spec: `docs/ops/specs/tools/lint/response.md`. Enforces strict fenced-envelope intake for machine markdown ingress in `--mode=dp|audit|architect|analyst|foreman`, applies marker and contamination-token rejection, rejects architect/analyst/foreman role-drift markers, enforces foreman addendum-structure checks plus decision-field coherence when present, and delegates structural DP validation in `dp` and `architect` modes. |
