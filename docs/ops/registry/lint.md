...
| LINT-17 | Response Lint | tools/lint/response.sh | Spec: `docs/ops/specs/tools/lint/response.md`. Enforces strict fenced-envelope intake for machine markdown ingress in `--mode=dp|audit|architect|analyst`, applies marker and contamination-token rejection, rejects architect and analyst role-drift markers, and delegates structural DP validation in `dp` and `architect` modes. |
