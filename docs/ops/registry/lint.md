...
| LINT-04 | Factory Lint | tools/lint/factory.sh | Spec: `docs/ops/specs/tools/lint/factory.md`. Verifies agent/skill/task registry synchronization, pointer integrity, and census coverage against `docs/ops/registry/factory.md`. |
...
| LINT-07 | Style Lint | tools/lint/style.sh | Spec: `docs/ops/specs/tools/lint/style.md`. Rejects markdown contractions across tracked documentation surfaces and enforces audit-versus-foreman mode split guard lines, auditor output marker/no-citations lines, compatibility alias deprecation key lines in the bundle policy manifest, and canonical OPEN marker contract lines in `ops/bin/open` (with legacy marker regression blocked). |
...
| LINT-17 | Response Lint | tools/lint/response.sh | Spec: `docs/ops/specs/tools/lint/response.md`. Enforces strict fenced-envelope intake for machine markdown ingress in `--mode=dp|audit|architect|analyst|foreman|conformist`, aligns cross-stance freeze keys in `ops/src/shared/stances.json`, applies marker and contamination-token rejection, rejects architect/analyst/foreman/conformist role-drift markers, enforces architect ask-back fallback rejection (including observed variant phrasing), enforces foreman addendum-structure checks plus decision-field coherence when present, and delegates structural DP validation in `dp`, `architect`, and `conformist` modes with deterministic architect delegate-on fixtures. |
| LINT-18 | Debt Lint | tools/lint/debt.sh | Spec: `docs/ops/specs/tools/lint/debt.md`. Validates `ops/lib/manifests/DEBT.md` row schema, enforces required lifecycle fields (`added_in`, `owner`, `remove_by_dp`, `reason`), and fails on stale active debt past removal packet threshold. |
