...
| LINT-11 | RESULTS Lint | tools/lint/results.sh | Spec: `docs/ops/specs/tools/lint/results.md`. Verifies certification RESULTS schema, template hash parity, and Contractor Execution Narrative/Decision Leaf requirements; git hash parity is strict in explicit-path mode, while no-arg and `--all` modes perform historical structural scans. |
