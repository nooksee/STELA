...
* [ops/specs/binaries/dump.md](ops/specs/binaries/dump.md) — Dump binary specification. Includes Scope Taxonomy: named scope definitions (`core`, `platform`, `factory`) and worker baseline (`dp+allowlist`) semantics. Default recommended operator audit scope: `--scope=core` (excludes `projects/` and `opt/_factory/`). Use `--scope=platform` or `--scope=factory` only when explicitly required.
