...
Both legacy and v2 closing sidecar schemas are accepted; `ops/bin/certify` is the schema authority; `tools/lint/style.sh` passes both.
...
`tools/lint/results.sh` accepts both legacy and v2 Mandatory Closing Block schemas emitted by `ops/bin/certify`; certify remains the schema authority for RESULTS closing-block labels.
