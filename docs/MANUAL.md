...
- External auditor receives: RESULTS receipt and native audit bundle artifacts generated by `./ops/bin/bundle --profile=audit --out=auto` (`BUNDLE-*.txt` and `BUNDLE-*.manifest.json`; `.tar` optional).
- External auditor does not receive by default: CDD artifacts, addendum-authorization bundles (`--profile=auditor`), or contractor-only intake artifacts.
