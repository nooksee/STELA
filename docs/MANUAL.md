...
| `audit` | `./ops/bin/bundle --profile=audit --out=auto` | `BUNDLE-*.txt`, `BUNDLE-*.manifest.json`, DP RESULTS receipt | Audit stance is PASS/FAIL verdict only. |
| `foreman` | `./ops/bin/bundle --profile=foreman --intent="ADDENDUM REQUIRED: <DECISION_ID> - <ONE-LINE BLOCKER>" --out=auto` | `BUNDLE-*.txt`, `BUNDLE-*.manifest.json` | Addendum authorization intake only; not used for PASS/FAIL verdicts. |
...
| `conform` | `./ops/bin/bundle --profile=conform --out=auto` | `BUNDLE-*.txt`, `BUNDLE-*.manifest.json`, draft DP input | Conformist stance normalizes structure without changing intent. |
...
- External reviewer receives: RESULTS receipt and native audit bundle artifacts generated by `./ops/bin/bundle --profile=audit --out=auto` (`BUNDLE-*.txt` and `BUNDLE-*.manifest.json`; `.tar` optional).
