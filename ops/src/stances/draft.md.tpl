...
* Require the delivered bundle manifest to state `resolved_profile=draft`; if not, **STOP** and request a correct draft bundle.
* Logic: `PoT.md`. Structure: `ops/src/surfaces/dp.md.tpl`.
* Treat the bundle-provided `PLAN.md` body as the governing scope.
* Treat the bundle-provided `[DP AUTHORING SCAFFOLD]` block as the canonical DP structure to preserve.
...
* You may make the smallest bridge decision needed to realize the settled plan when delivered bundle artifacts settle intent and authority.
* Preserve canon-owned text from the authoring scaffold unless directly visible bundle artifacts require a bounded correction inside settled scope.
...
3. **AUTHOR THE DP FROM THE SCAFFOLD**:
   * Use the bundle-provided authoring scaffold as the base document.
   * Preserve the scaffold's canon-owned section order, headings, injected rules, and closeout boilerplate.
   * Replace only the authoring markers and packet-specific section content using `PLAN.md` plus directly visible bundle artifacts.
4. **CONTENT REQUIREMENTS**:
   ...
   * `3.4.2 Request`: translate PLAN into worker requirements using the settled plan body as governing scope and use directly visible bundle artifacts to correct stale or self-contradictory request details and make bounded continuity decisions needed for a usable DP inside that scope.
...
5. **CONSTRAINTS**:
   ...
   * Do not infer objective or authority beyond delivered bundle artifacts. For continuity details inside settled scope, make the smallest repo-local decision needed for a usable DP and state it plainly as continuity rather than direct inspection.
   * When directly visible bundle artifacts show repo/runtime contract drift inside the settled plan, name the defect plainly and encode the corrective work in the DP instead of flattening the output into read-only summary.
