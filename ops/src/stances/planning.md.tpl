...
* Require bundle-provided `storage/handoff/TOPIC.md` as the planning input source.
* Use delivered bundle evidence first.
...
* Treat the immediate packet as explicit only if:
  ...
  * the delivered bundle evidence directly requires a first packet ordering, or
...
Steps:
0. If bundle-provided `storage/handoff/TOPIC.md` is missing: STOP and report the missing topic artifact.
1. Read the bundle-provided topic and directly visible bundle evidence first.
2. Determine whether the topic includes multiple independent work families.
3. Determine whether the immediate packet is explicitly identified by the topic, delivered bundle evidence, or user priority.
...
For machine-ingest planning mode: require bundle-provided `storage/handoff/TOPIC.md`; do not use inline query fallback.
For machine-ingest planning mode: use delivered bundle evidence first.
For machine-ingest planning mode: do not add repository-operating details, workflow examples, command families, or GitHub action lists unless they are directly visible in delivered bundle artifacts.
...
For machine-ingest planning mode: treat the immediate packet as explicit only if the topic directly names the first packet or first work family, the delivered bundle evidence directly requires a first packet ordering, or the user explicitly prioritizes one work family.
...
For machine-ingest planning mode: when the relevant repo surfaces are delivered and directly visible, make concrete repo-specific claims grounded in those artifacts instead of retreating to generic high-level language.
...
For machine-ingest question mode: mark at most one substantive option `(Recommended)` only when delivered bundle evidence justifies it; never mark the redirect option `(Recommended)`.
...
For machine-ingest question mode: if topic text is present but intent cannot be settled from delivered bundle evidence, ask the minimum clarifying questions needed rather than forcing a final plan.
...
For final plan mode: when the needed repo surfaces are delivered and directly visible, use them to make the handoff concrete rather than artificially flattening the plan to generic language.
