...
path_is_registered_retired_factory_definition() {
  local path="$1"
  local registry="${DP_FACTORY_REGISTRY_OVERRIDE:-docs/ops/registry/factory.md}"
  ...
  awk -F'|' -v target="$path" '
    ...
    }
    {
      ...
      if (former_path != target) {
        next
      }

      expected_path=""
      if (kind == "agent" && id ~ /^R-AGENT-[0-9][0-9]$/) {
        expected_path="opt/_factory/agents/" tolower(id) ".md"
      } else if (kind == "skill" && id ~ /^S-LEARN-[0-9][0-9]$/) {
        expected_path="opt/_factory/skills/" tolower(id) ".md"
      } else if (kind == "task" && id ~ /^B-TASK-[0-9][0-9]$/) {
        expected_path="opt/_factory/tasks/" tolower(id) ".md"
      }
...
check_allowlist_pointer_integrity() {
  ...
  for entry in "${entries[@]}"; do
    ...
    if [[ ! -e "${REPO_ROOT}/${normalized}" ]]; then
      ...
      # Certified historical packets may name Factory definitions that were later
      # retired. Accept only an exact kind, ID, and former-path match from the
      # canonical retirement ledger so absent-path typos still fail closed.
      if path_is_registered_retired_factory_definition "${normalized}"; then
        continue
      fi
...
run_test() {
  ...
  tmp_retirement_registry="$(mktemp)"
  printf '%s\n' \
    '## Retired Definitions' \
    '' \
    '| Kind | ID | Former Path | Retired In | Reason |' \
    '| --- | --- | --- | --- | --- |' \
    '| agent | R-AGENT-42 | opt/_factory/agents/r-agent-42.md | TEST | fixture |' \
    > "$tmp_retirement_registry"

  if ! DP_FACTORY_REGISTRY_OVERRIDE="$tmp_retirement_registry" path_is_registered_retired_factory_definition "opt/_factory/agents/r-agent-42.md"; then
    rm -f "$tmp_retirement_registry"
    echo "FAIL: --test expected exact retired Factory definition to pass" >&2
    exit 1
  fi

  if DP_FACTORY_REGISTRY_OVERRIDE="$tmp_retirement_registry" path_is_registered_retired_factory_definition "opt/_factory/agents/r-agent-41.md"; then
    rm -f "$tmp_retirement_registry"
    echo "FAIL: --test expected unregistered retired Factory path to fail" >&2
    exit 1
  fi
  rm -f "$tmp_retirement_registry"
