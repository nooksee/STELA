...
check_allowlist_pointer_integrity() {
  ...
  for entry in "${entries[@]}"; do
    ...
    if [[ ! -e "${REPO_ROOT}/${normalized}" ]]; then
      ...
      # Allow committed deletions (CI clean-checkout where working-tree and cached diffs
      # are both empty because the deletion is already part of the branch HEAD).
      local _lsf _log
      _lsf="$(git ls-files -- "${normalized}")"
      _log="$(git log --oneline -1 -- "${normalized}")"
      if [[ -z "$_lsf" && -n "$_log" ]]; then
        continue
      fi
