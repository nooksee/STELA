...
check_task_dashboard_v2() {
  ...
  for ((i=0; i<${#labels[@]}; i++)); do
    ...
    if [[ -z "$line" ]]; then
      fail "TASK v2 missing heading '${labels[i]}'"
      missing=1
      heading_lines+=("")
    else
      heading_lines+=("$line")
  ...
  if (( !missing )); then
    for ((i=0; i<${#heading_lines[@]}-1; i++)); do
      if (( heading_lines[i] >= heading_lines[i+1] )); then
        fail "TASK v2 headings out of order: '${labels[i]}' should appear before '${labels[i+1]}'"
        break
      fi
    done
  fi
  ...
  if ! grep -Eq '^Pointer:[[:space:]]*storage/handoff/OPEN-<branch>-<short-hash>[.]txt([[:space:]]*[(].*[)])?$' "$path" \
    ...
    fail "TASK v2 Session State pointer must be pointer-first and exact"
  fi
  if ! grep -nE '^Context Manifest:[[:space:]]*ops/lib/manifests/CONTEXT\.md' "$path" >/dev/null; then
    fail "TASK v2 missing Session State Context Manifest pointer"
  fi
  ...
  if grep -nE '^[[:space:]]*(Active Branch|Branch|Base HEAD):' <<< "$session_block" >/dev/null; then
    fail "TASK v2 Session State must not mirror inline branch/hash fields"
  fi
  if grep -nE 'work/[[:alnum:]_.-]+' <<< "$session_block" >/dev/null; then
    fail "TASK v2 Session State must not include work branch mirrors"
  fi
  if grep -nE '[0-9a-f]{7,}' <<< "$session_block" >/dev/null; then
    fail "TASK v2 Session State must not include hash mirrors"
  fi
  ...
  if ! grep -Fq 'AMENDMENT:' "$path" && ! grep -Eq '^AMENDMENT[[:space:]]*[(]' "$path"; then
    fail "TASK v2 Freeze Rule must require AMENDMENT entries for post-dispatch changes"
  fi
  ...
  if [[ "$canon_count" != "6" ]]; then
    fail "TASK v2 canon load order must contain exactly six numbered items"
  fi
  if grep -nE '(tools/|ops/bin/|docs/ops/specs/)' <<< "$canon_block" >/dev/null; then
    fail "TASK v2 canon load order is bloated; tools/binaries belong in DP-scoped load order"
  fi

  if ! grep -Fq 'Worker does not read OPEN.' "$path"; then
    fail "TASK v2 must state that workers do not read OPEN"
  fi
  ...
  ' "$path")"; then
    ...
    fail "TASK v2 contains forbidden OPEN-reading instruction: ${open_read_issue}"
  fi
  ...
  if ! grep -Fq 'git diff --name-only' <<< "$receipt_block"; then
    fail "TASK v2 receipt contract missing git diff --name-only proof"
  fi
  if ! grep -Fq 'git diff --stat' <<< "$receipt_block"; then
    fail "TASK v2 receipt contract missing git diff --stat proof"
  fi
  if ! grep -iq 'required pasted outputs' <<< "$receipt_block" \
    && ! grep -iq 'paste outputs' <<< "$receipt_block"; then
    fail "TASK v2 receipt contract missing required pasted outputs clause"
  fi
  ...
  if ! grep -nF 'Mandatory Closing Block' "$path" >/dev/null; then
    fail "TASK v2 closeout block missing Mandatory Closing Block heading"
  fi
  ...
  for label in "${closing_labels[@]}"; do
    if [[ "$label" == "Final Squash Stub (plaintext) (Must differ from #1)" ]]; then
      if ! grep -Fxq "$label" "$path" \
        ...
        fail "TASK v2 closeout block missing label '${label}'"
      fi
      continue
    fi
...
check_task_dashboard_v2 "TASK.md"
